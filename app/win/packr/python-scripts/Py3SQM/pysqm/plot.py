...
class Ephemerids(object):
    ...
    def calculate_moon_ephems(self, thedate):
        ...
        # Moon rise and set
        self.moon_prev_rise = self.ephem_date_to_datetime(
            self.Observatory.previous_rising(ephem.Moon())
        )
        self.moon_prev_set = self.ephem_date_to_datetime(
            self.Observatory.previous_setting(ephem.Moon())
        )
        self.moon_next_rise = self.ephem_date_to_datetime(
            self.Observatory.next_rising(ephem.Moon())
        )
        self.moon_next_set = self.ephem_date_to_datetime(
...
class SQMData(object):
    ...
    def extract_metadata(self, raw_data_and_metadata):
        ...
        metadata_lines = [
            line for line in raw_data_and_metadata if format_value(line)[0] == "#"
        ]

        # Extract the serial number
        serial_number_line = [
            line for line in metadata_lines if "SQM serial number:" in line
        ][0]
    ...
    def load_rawdata(self, filename):
        ...
        self.raw_data = [
            line for line in raw_data_and_metadata if self.check_validdata(line) == True
        ]
        sqm_file.close()
    ...
    def check_number_of_nights(self):
        ...
        if np.size(self.premidnight.localdates) > 0:
            ...
            self.Night = np.unique(
                [
                    (DT - timedelta(hours=12)).date()
                    for DT in self.aftermidnight.localdates
                ]
            )[0]
        else:
    ...
    def data_statistics(self, Ephem):
        ...
        Stat.bests_err = Stat.bests_std * 1.0 / np.sqrt(Stat.bests_number)

        Stat.model_nterm = round(1 + Stat.number / 25)
        # data_smooth = fourier_filter(self.astronomical_night_sb,nterms=Stat.model_nterm)
        data_smooth = window_smooth(
            self.astronomical_night_sb, window_len=Stat.model_nterm
        )
        min_length = min(len(data_smooth), len(self.astronomical_night_sb))
        data_residuals = (
            self.astronomical_night_sb[:min_length] - data_smooth[:min_length]
        )
...
class Plot(object):
    ...
    def make_subplot_sunalt(self, twinplot=0):
        ...
        self.thegraph_sunalt.set_ylabel(
            "Sky Brightness (mag/arcsec2)", fontsize="medium"
        )
        ...
        tick_values = list(
            range(config.limits_sunalt[0], config.limits_sunalt[1] + 5, 5)
        )
        ...
        self.thegraph_sunalt.grid(
            True, which="major", alpha=0.2, color="k", ls="", lw=0.5
        )
        self.thegraph_sunalt.grid(
            True, which="minor", alpha=0.2, color="k", ls="solid", lw=0.5
        )
    ...
    def plot_data_sunalt(self, Data, Ephem):
        ...
        if np.size(TheData.filter) > 0:
            self.thegraph_sunalt.plot(
                ...
                np.array(TheData.night_sbs)[TheData.filter],
                color="#2ca02c",
            )
            """
            self.thegraph_sunalt.plot(\
        ...
        self.thegraph_sunalt.set_xlim(
            [
                config.limits_sunalt[0] * np.pi / 180.0,
                config.limits_sunalt[1] * np.pi / 180.0,
            ]
        )
        self.thegraph_sunalt.set_ylim(config.limits_nsb)

        premidnight_label = (
            str(Data.premidnight.label_dates).replace("[", "").replace("]", "")
        )
        aftermidnight_label = (
            str(Data.aftermidnight.label_dates).replace("[", "").replace("]", "")
        )
        ...
        if np.size(Data.Night) == 1:
            self.thegraph_time.text(
                ...
                % (
                    Ephem.moon_phase,
                    "%",
                    Ephem.moon_maxelev * 180.0 / np.pi,
                    "$^\mathbf{o}$",
                ),
                color="black",
                fontsize="small",
...
if __name__ == "__main__":
    ...
    try:
        ...
        config = settings.GlobalConfig.config
        make_plot(
            input_filename=InputArguments.input, send_emails=False, write_stats=True
        )
    except:
        raise
