...
class device(observatory):
    def standard_file_header(self):
        ...
        header_content = header_content.replace("$DEVICE_ID", str(config._device_id))
        header_content = header_content.replace(
    ...
    def define_filenames(self):
        ...
        # "_"+config._observatory_name+"_"+yearmonthday+".csv"
        self.daily_datafile = (
        ...
        while True:
            try:
                ...
                if buf[3].encode("hex") == "f7":
                    print(
                        "Received from %s: MAC: %s" % (addr, buf[24:30].encode("hex"))
                    )
            except:
